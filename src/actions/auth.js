...
export const register =
  ({ name, email, password, passwordConfirm, token }) =>
  async (dispatch) => {
    try {
      dispatch({
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    const userImage = await axios.get(`/api/s3UserImageById/${id}`);
    ...
    // axios
    //   .all([
    //     axios.get(`/api/users/getUserDetails/${id}`),
    //     axios.get(`/api/s3UserImageById/${id}`),
    //   ])
    //   .then(
