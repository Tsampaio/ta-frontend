...
export const getCourse = (courseTag, userId) => async (dispatch, getState) => {
  ...
  try {
    ...
    const body = userId
      ? JSON.stringify({ courseTag, userId })
      : JSON.stringify({ courseTag });
    console.log(body);
...
export const finishLessonAction =
  (lesson, courseId, token) => async (dispatch) => {
    try {
...
export const deleteVideoClassAction =
  (courseId, classId) => async (dispatch, getState) => {
    try {
      dispatch({
