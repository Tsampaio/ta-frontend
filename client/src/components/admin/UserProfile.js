...
const Courses = ({ match, history }) => {
  ...
  const allPurchases = userPurchases.map((purchase, i) => {
    ...
    const products = purchase.productName.map((product, i) => {
      ...
      return (
        <>
          <span>{product}</span>
          {purchase.productName.length - 1 !== i ? <hr /> : ''}
        </>
      );
    });
  ...
  const allCourses = all.map((course, i) => {
    return (
      <option key={i} value={`${course._id},${course.name}`}>
        {course.name}
      </option>
    );
  });
  ...
  const displayUserCourses = userCoursesDetails.map((course, i) => {
    return (
      <tr key={i}>
        <td>{course.name}</td>
        <td onClick={() => removeCourse(course._id, course.name)}>
  ...
  const userLastLoginDate = `${userLastLogin.getDate()}/${
    userLastLogin.getMonth() + 1
      ...
      <div className={`${styles.adminCtn} col-xl-10`}>
        ...
        <div className="row">
          ...
          <div className="col-10">
            <table>
              ...
              <tbody>{allPurchases.reverse()}</tbody>
            </table>
          </div>
        </div>
        <div className="row">
        ...
        <div className="row">
          <div className="col-2">
            ...
            <p>
              Manually enroll this user in a new course. Users are not charged
              for manual enrollments.
            </p>
          </div>
          <div className="col-10">
            <div className={`${styles.card} ${styles.userCard}`}>
